...
class Brain(object):
    def __init__(self):
        ...
        self.position_config = {'table_card' : [120, 470, 425, 555]}
...
brain = Brain()
brain.cards_on_table = []
table_card_img = Image.open('../pics/test.png')

# cv2.imshow('Detected', np.array(table_card_img))
# cv2.waitKey(0)
# cv2.destroyAllWindows()
for key, template in brain.card_images.items():
    count, points, best_fit, min_val = brain.match_template(table_card_img, template)
    if count >= 1:
        brain.cards_on_table.append(key)

brain.game_stage = ''

if len(brain.cards_on_table) < 1:
    brain.game_stage = "PreFlop"
elif len(brain.cards_on_table) == 3:
    brain.game_stage = "Flop"
elif len(brain.cards_on_table) == 4:
    brain.game_stage = "Turn"
elif len(brain.cards_on_table) == 5:
    brain.game_stage = "River"
print ('table stage', brain.game_stage, 'cards on table', brain.cards_on_table)